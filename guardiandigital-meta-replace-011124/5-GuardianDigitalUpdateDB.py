    ...
    if extracted_text['map_id'] in map_dic:
        ...
        if 'w5zxq_menu' in map_dic[extracted_text['map_id']][2]:
            try:
                if extracted_text['Extracted Text'] in menu_table['alias'].values:
                    # link_value = menu_table.loc[menu_table['alias'] == extracted_text['Extracted Text'], 'link'].values[0]
                    Title_link_value_id = menu_table.loc[(menu_table['alias'] == extracted_text['Extracted Text']), 'id'].values[0]
                    # print(metaMetaTable)
...
infodf['Title_id'] = infodf['Title_Link'].apply(lambda link: link.split('=')[-1] if '=' in link else '')
infodf['Title_com'] = infodf['Title_Link'].apply(lambda link: 'w5zxq_' + link.split('com_')[1].split('&')[0]  if 'com_' in link else '')
...
for h1_id_value, h1_com_value, meta_id_value, meta_com_value, map_id, map_data, meta_tag, H1_tag, new_H1, old_h1, new_title, old_title, old_Meta, New_Meta,title_com_value,title_id_value in zip( 
    ...
    infodf['Title_com'],infodf['Title_id']):
    ...
    try:
                ...
                for i, row in df.iterrows():
                    ...
                    if 'sppagebuilder' in h1_com_value:
                        ...
                        if updated_string != text and len(old_h1) != 0:
                            ...
                            logging.info(f'Sppagebuilder Table h1: {h1_id_value}-{old_h1}{" " * spacing}{new_H1}')
                    ...
                    # 2 _content --> replace of title
                        ...
                        if updated_string != params :
                            infodf.loc[(infodf['map_id'] == map_id) & (infodf['map_data'] == map_data ), 'H1_Updated_DB'] = True
                            logging.info(f'w5zxq_menu: {h1_id_value}-{old_h1}{" " * spacing}{new_H1}')
        ...
        if 'w5zxq_menu' in title_com_value:
            ...
            if updated_string != params :
                update_stmt = update(tablemenu).where(tablemenu.c.id == int(title_id_value)).values(params=updated_string )
                with engine.begin() as connection:
                    connection.execute(update_stmt)
                    logging.info(f'w5zxq_menu title: {title_id_value}-{old_title}{" " * 20}{new_title}')
                conn.commit()
...
def update_fields_values_table(article_ids):
    logger = get_logger(name="myfle.txt", log_file="mylog.info",log_level=logging.INFO) 
